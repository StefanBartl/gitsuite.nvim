-- scripts/minimal_init.lua — headless test bootstrap for gitsuite.nvim.
--
...
--- lib.nvim and diff.nvim are hard runtime dependencies (see README, LUA-01),
--- not optional ones -- gitsuite.nvim's command/composer/adapter layers
--- `require("lib.*")`/`require("diff.*")` directly, so the specs cannot run
--- without both on the rtp. plenary.nvim is the busted-compatible test
--- harness the specs under TESTS/ are written against.
---
--- Three ways each can be found, in descending order of explicitness: an
--- explicit env var, a `.deps/<name>` checkout (what CI uses -- see
--- .github/workflows/ci.yml), or a sibling checkout next to this repo
--- (`../lib.nvim`, ...) for a local contributor who already has it cloned
--- that way.
...
add_dep("DIFF_NVIM_DIR", "diff.nvim", "diff.core")
