---@module 'gitsuite.features.browse.url'
--- Pure remote-URL parsing and building for `:Git browse`: no `vim.api`
--- calls, no shelling out, headless-testable (PRINCIPLES.md "Pure Core /
--- Impure Shell"). `features/browse/init.lua` is the impure shell around it
--- (buffer/cursor reads, the actual git calls, opening the browser).
...
---built in; anything else is looked up in `hosts_cfg` (`cfg.browse.hosts`).
---@param host string
...
function M.host_kind(host, hosts_cfg)
  ...
  if host == "gitlab.com" then return "gitlab" end
  if host == "codeberg.org" then return "codeberg" end
  return hosts_cfg[host]
end

---Build a web URL for a file (optionally with a line anchor) or, with
---`rel_path == nil`, the repository root. `rel_path`/`branch` are
---git-controlled values (SEC-42), never raw user text, but this still only
...
function M.build(kind, remote, branch, rel_path, first, last)
  ...
  if not rel_path then return base end
